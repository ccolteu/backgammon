...
object Engine {
  ...
  private fun pickBeginner(plays: List<Play>, side: Side, random: Random): Play {
  ...
  private fun bestOnePly(plays: List<Play>, side: Side): Play {
    val candidates = plays.sortedByDescending { fitness(side, evaluateWhiteAdvantage(it.after)) }.take(LOOKAHEAD_CANDIDATES)
  ...
  private fun bestOnePlyPubEval(plays: List<Play>, race: Boolean): Play {
    val candidates = plays.sortedByDescending { PubEval.score(it.after, race) }.take(LOOKAHEAD_CANDIDATES)
