...
import com.cc.backgammon.data.PrefsGameStore
import com.cc.backgammon.data.isResumable
import com.cc.backgammon.domain.GameState
import com.cc.backgammon.domain.Move
...
import com.cc.backgammon.domain.boardWithoutMover
import com.cc.backgammon.domain.dieSpent
import com.cc.backgammon.domain.startingGame
import com.cc.backgammon.domain.turnStatus
import com.cc.backgammon.engine.AiLevel
import com.cc.backgammon.engine.Engine
...
class BackgammonViewModel(
  ...
  private fun startCpuPlanning() {
    val generation = ++planGeneration
    val snapshot = game
    val level = aiLevel
    phase = PlayPhase.THINKING
    cpuQueue.clear()
    publish()
    planJob?.cancel()
    planJob =
      viewModelScope.launch {
        val planned = withContext(computeDispatcher) { Engine.planTurn(snapshot, level) }
        if (generation != planGeneration) return@launch
        if (planned.isEmpty()) {
          phase = PlayPhase.NO_MOVES
          publish()
        } else {
          cpuQueue.addAll(planned)
          startNextCpuMove()
        }
      }
  }

  private fun abandonCpuPlan() {
    planGeneration += 1
    planJob?.cancel()
    planJob = null
    cpuQueue.clear()
  ...
  private fun toUi(): GameUiState {
    ...
    return GameUiState(
      ...
      newGameEnabled = game != startingGame(),
      aiLevel = aiLevel,
