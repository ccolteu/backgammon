...
class EngineTest {
  ...
  @Test
  fun beginnerAdvancedAndExpert_planLegalTurns() {
    val rolled = Rules.withDice(startingGame().copy(sideToMove = Side.BLACK), 3, 5)
    for (level in listOf(AiLevel.BEGINNER, AiLevel.ADVANCED, AiLevel.EXPERT)) {
      var current = rolled
  ...
  @Test
  fun intermediate_hitsAndCoversInsteadOfLeavingABlot() {
    val game =
      position(
        white = mapOf(20 to 1),
  ...
  @Test
  fun intermediate_holdsAnAnchorWhenAheadInTheRace() {
    val game =
      position(
  ...
  @Test
  fun intermediate_makesTheFivePointInsteadOfHittingALooseBlot() {
    val game =
      position(
        white = mapOf(15 to 1),
  ...
  @Test
  fun intermediate_doesNotBreakAPointToSlotInFrontOfAPrime() {
    val game =
      position(
  ...
  @Test
  fun advanced_hitsAndCoversLikeIntermediate() {
    val game =
      position(
        white = mapOf(20 to 1),
        black = mapOf(19 to 2, 18 to 1, 13 to 12),
    ...
    val after = playOut(game, AiLevel.ADVANCED)
    assertEquals(2, after.countAt(20))
    assertEquals(1, after.whiteBar)
  }
